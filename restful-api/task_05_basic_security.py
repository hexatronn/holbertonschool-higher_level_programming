...
def handle_fresh_required(jwt_header, jwt_payload):
...
def login():
    ...
    user = users.get(username)
    if not user or not check_password_hash(user.get("password"), password or ""):
...
@jwt_required()
def admin_only():
    """Admin-only route requiring JWT and role check."""
